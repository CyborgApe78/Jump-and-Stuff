...
func mouse_check() ->bool:
	if mouse_check_delay:
		if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) || Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT) || Input.is_mouse_button_pressed(MOUSE_BUTTON_MIDDLE):
			return false
	...
	while c != null && c is Control:
		...
		c = c.get_parent()
	var canvas_rect :Rect2 = Rect2(canvas_pos + 20*Vector2.ONE,viewport.get_parent().size-36*Vector2.ONE)
	...
	if !result && (Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) || Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT) || Input.is_mouse_button_pressed(MOUSE_BUTTON_MIDDLE)):
...
static func find_viewport_2d(plugin :EditorPlugin) ->Node:
	var canvas :Node = plugin.get_editor_interface().get_editor_main_screen().get_child(0)
	var result :Node = canvas.get_child(1).get_child(0).get_child(0).get_child(0).get_child(0)
